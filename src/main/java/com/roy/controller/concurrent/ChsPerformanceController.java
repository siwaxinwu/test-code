...
@RequestMapping("chs")
@Slf4j
public class ChsPerformanceController {
    ...
    public String good() throws InterruptedException {
        ...
        stopWatch.stop();
        /*try {
            Assert.isTrue(normaluse.size() == 5, "normaluse size error");
        }catch (Exception e){
            e.printStackTrace();
        }*/
    ...
    private Map<String, Long> normaluse() throws InterruptedException {
        ...
        forkJoinPool.execute(() -> IntStream.rangeClosed(1, 10000000).parallel().forEach(i -> {
                    String key = "item" + ThreadLocalRandom.current().nextInt(ITEM_COUNT); //10
                    synchronized (freqs) {
